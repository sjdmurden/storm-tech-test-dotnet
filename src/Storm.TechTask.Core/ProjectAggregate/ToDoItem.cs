...
namespace Storm.TechTask.Core.ProjectAggregate
{
    ...
    public class ToDoItem : BaseEntity
    {
        ...
        public ToDoItem(string title, string description, bool isDone) 
        { 
            this.Title = title; this.Description = description; this.IsDone = isDone; 
        }
        public void MarkComplete()
        {
            this.IsDone = true; // This would be a good place for another domain event
        }
        public override string ToString() 
        { 
            string status = this.IsDone ? "Done!" : "Not done."; return $"{Id}: Status: {status} - {Title} - {Description}"; 
        } 
    }  
}*/